...
def read_control_file(control_file_path):
    ...
    if not os.path.exists(control_file_path):
        return {"paused": False, "speed": 1.0}
    
    try:
        with open(control_file_path, 'r', encoding='utf-8') as f:
            control_data = json.load(f)
            return {
                "paused": control_data.get("paused", False),
...
class DroneSimulation:
    ...
    def __init__(self, allocation_result, target_area=(80, 80), area_size=100):
        ...
        # 控制文件路径（使用当前工作目录，由Qt设置）
        self.control_file_path = os.path.join(os.getcwd(), 'simulation_control.json')
        print(f"[调试] 控制文件路径: {self.control_file_path}")
    ...
    def update_positions(self, steps=100):
        ...
        for step in range(steps):
            # 读取控制状态
            control_state = read_control_file(self.control_file_path)
            ...
            # 根据倍速调整暂停时间
            speed_multiplier = control_state.get("speed", 1.0)
            ...
            if step % 5 == 0:
                self.draw_drones()
                # 根据倍速调整暂停时间
                pause_time = 0.1 / speed_multiplier if speed_multiplier > 0 else 0.1
